// ============================================================
//  ColmillosDelAlba Bot  —  Versión mejorada y depurada
// ============================================================
...
client.on(Events.GuildMemberAdd, async (member) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage('https://i.imgur.com/a/welcome-s-Vn8N5Zc')
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'modal_reclutamiento') {
    ...
    return interaction.reply({ content: '✅ Tu solicitud fue enviada. El Staff la revisará pronto.', ephemeral: true });
  }
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'reglas') {
      const texto = [
        ...
        '> • ❌ No envíes links de servidores externos (puedes consultarlo).',
        '',
        '### 3️⃣ 🔹 **Actividad**',
  ...
  if (interaction.customId === 'crear_ticket') {
    ...
    const embedFormulario = new EmbedBuilder()
      ...
      .setDescription(
        ...
        'Buscamos jugadores activos, amigables y dispuestos a aportar.\n' +
    ...
    await nuevoCanal.send({
      content:    `<@&${CONFIG.STAFF_TICKETS_ID}> <@${interaction.user.id}>`,
  ...
  // Botón: Abrir formulario (modal)
  if (interaction.customId === 'abrir_formulario') {
    const campos = [
      new TextInputBuilder().setCustomId('f_datos').setLabel('NICK / EDAD / GÉNERO / PAÍS').setPlaceholder('Ej: 1fsi / 16 / Masculino / Uruguay').setStyle(TextInputStyle.Short).setRequired(true),
      new TextInputBuilder().setCustomId('f_especialidad').setLabel('ESPECIALIDAD Y NIVEL').setPlaceholder('Ej: PvP: Medio, o Constructor: Alto').setStyle(TextInputStyle.Short).setRequired(true),
